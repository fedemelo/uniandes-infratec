...
int main(){
    ...
    while (bool_invalid_size) {
        ...
        if (tamano > 0) {  // Valido que n > 0.
            ...
            printf("Error: El tamano del vector debe ser mayor que 0. Por favor vuelva a intentarlo.\n");
        }
    }
    ...
    // PEDXr cadena a procesar. Se la pide al usuario hasta que el usuario escriba una cadena con caracteres correctos.
    ...
    while (!cadena_aceptada) {
        ...
        cadena_aceptada = empaquetar(final, cadena, tamano);

        if (!cadena_aceptada) {
            printf("Error: La cadena ingresada contiene caracteres no aceptables. Por favor vuelva a intentarlo.\n");
        }
    }
...
int empaquetar(int* fina, char* caden, int taman){
    char cadena_aceptada = 1;
    __asm{
        
        mov EAX, fina  // EAX: Apuntador a vector de enteros.
        mov EBX, caden  // EBX: Apuntador a cadena de caracteres.

        mov ESI, taman  
        imul ESI, 4  // ESI: Tamano del vector de enteros por 4.
        ...
        for:
            ...
            jg noEsMayuscula
                ...
                add [EBX], 9
                and [EBX], 0fh 
                mov CL, [EBX]  // Variable temporal 1 CL          
                jmp hecho
            ...
            jg noEsNumero
                ...
                /* Si si es numero, a variable temporal */
                and [EBX], 0fh
                mov CL, [EBX] //variable temporal 1 CL   
                jmp hecho 
            ...
            noEsNull:  // Ni numero ni mayuscula ni null
                mov cadena_aceptada, 0  // No se acepta la cadena

            hecho:
                shl CL, 4  // Corrimiento no aritmetico 4 a la derecha. Queda abcd0000
                inc EBX  // Se pasa al siguiente caracter

            /* Lo mismo que antes, pero con el otro catacter de la pareja */
